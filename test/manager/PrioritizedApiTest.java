...
public class PrioritizedApiTest {
    ...
    public void shouldReturnHistory() throws IOException, InterruptedException {
        ...
        List<Task> prioritized = gson.fromJson(body, new TaskListTypeToken().getType());

        assertEquals(2, prioritized.size(), "Некорректное количество задач.");
        assertEquals("Task1", prioritized.get(0).getName(), "Некорректное имя задачи.");
        assertEquals(List.of(task1, task2), prioritized, "Задачи не совпадают.");
    }
